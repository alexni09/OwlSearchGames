<!--
    A Drag-and-drop four-state switch (for game difficulty).
-->
<script setup>
...
const exec = (e) => {
    switch (e.keyCode) {
        ...
        case 39: myState.value < 2 ? myState.value++ : myState.value = 3; playSound(); emit('update:modelValue',myState.value); break;
    }
}
const onClick = (e) => { 
    myState.value < 3 ? myState.value++ : myState.value = 0
    playSound() 
    emit('update:modelValue',myState.value)
}
const playSound = () => { document.getElementById('gameSwitchAudio2').play() }
</script>
<template>
    <div class="relative">
        <audio id="gameSwitchAudio2" src="/audio/gameswitch2.mp3" preload="auto" />
        <div class="px-3 py-2 z-0 relative w-64 bg-neutral-100 rounded-xl border-2 border-neutral-300 hover:cursor-pointer" @click="onClick">
            <div class="relative w-[72px] outline-none" :tabindex="tabindex" @keydown="exec">
                <div class="relative z-10 w-[72px] h-6 bg-neutral-100 rounded-full border-2 border-neutral-300" />
        ...
        <div class="ml-2 mt-0.5 lg:mt-0 z-10 absolute left-[88px] top-[8px] text-sm lg:text-base"
            :class="{
